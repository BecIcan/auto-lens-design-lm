...
from eadld.optimization import parameterization as param, residuals as res


def _combine_ray_weights(wavelength_weights, pupil_weights):
    """合并光谱与瞳面权重；任一项缺省时直接使用另一项。"""
    if wavelength_weights is None:
        return pupil_weights
    if pupil_weights is None:
        return wavelength_weights
    return wavelength_weights * pupil_weights
...
class ImagingSystemModule(pl.LightningModule):
    ...
    def compute_residuals_dict_and_logs(
        ...
        ray_weights = _combine_ray_weights(self.wavelength_weights, pupil_weights)
    ...
    def compute_lens_design_residuals(
        ...
        for component in self.residuals:
            ...
            # First-order constraints
                ...
                # the residual is documented to take the *maximum* y
    ...
    def try_build_simulation_model(self, xy: torch.Tensor | None = None):
        ...
        Args:
            xy: Spot diagrams in image space (shape: [2, n_fields, n_rays, n_wavelengths, n_lens (1)]).
        """
        # the coherent path needs the full ray state (directions and
