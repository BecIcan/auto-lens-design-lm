...
from eadld.modeling import ray_analysis as ra, ray_initialization as ri, optics


# a Fresnel-type surface ties the zone count to the finest period,
...
def check_ray_wave_sampling(lens, ray_initialization, psf_sampler, wavelengths):
    ...
    # 此处只估算静态采样需求，不参与成像梯度；显式脱离计算图避免误导性警告。
    distance = lens.s[-1].detach().item()
    wavelength = float(min(wavelengths)) * 1e-6
    edges = lens.z[0, 0, :, 3][(lens.z[0, 0] != 0).any(dim=-1)].detach()
...
class OpticsSimulator(torch.nn.Module):
    ...
    def __init__(
        ...
        # absolute normalization is only defined for the coherent path.
    ...
    def build_optics_model(
        ...
        if self.psf_mode == "ray_wave":
            # "skew_uniform_zonal" is also deterministic and
            ...
            for message in check_ray_wave_sampling(
            ...
            # the weights are a pure function of the same arguments
...
class PSFSampler(torch.nn.Module):
    ...
    def forward_ray_wave(
        ...
        """由追迹光程计算标量 RayWave PSF。

        离散 Kirchhoff 模型为
        `U(P)=sum_j w_j K_j(P) exp(i k L_j(P))`，`PSF=|U|^2`。
        光线先回投到末表面顶点平面，再传播到公共物理像面网格。
        ...
        # plane; the output medium is air in the EADLD sequential convention.
        ...
        # 环带对齐采样的壳层面积不同，瞳面权重必须进入场、参考球和 piston。
        ...
        # Strehl 分母使用同一瞳面采样上的理想会聚球，数值积分误差可相互抵消。
        # 参考球收敛到光轴；若改用主光线，1.5° 时会引入约 3e-4 的偏差。
        axis_distance = (
        ...
        if self.psf_normalization == "strehl":
