...
def select_zone(table: torch.Tensor, zone_index: torch.Tensor):
    ...
    add by cjy: the one-hot form allocates O(n_rays * n_zones * n_columns) and
...
def evaluate_zonal_profile(
    ...
    if zone_index is None:
        ...
        # add by cjy: an explicit index evaluates one extended facet, which is
        # smooth everywhere and therefore always defined.
        is_inside = torch.ones_like(rho, dtype=torch.bool)

    parameters = select_zone(z, zone_index)  # add by cjy
...
def find_marching_distance_zonal(
    ...
    # add by cjy: the assembled profile is discontinuous at every zone boundary,
    ...
    # add by cjy: one table lookup per candidate instead of a one-hot mask.
