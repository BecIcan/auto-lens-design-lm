...
from eisoptx.modeling import ray_initialization as ri, ray_analysis as ra, optics
from eisoptx.optimization import parameterization as param
...
class HDOEPhaseResiduals(Residuals):
    """Keep a zonal HDOE on its generated optical-path branch.

    For harmonic order ``M`` at the design wavelength, adjacent zones differ
    by ``M * wavelength`` in optical path.  Removing that known branch offset
    must leave one constant wavefront piston per field and lens.
    """
    ...
    def __init__(
        ...
        Args:
            weight: Residual weight used by EISOPTX.
            ...
            name: Distinct name for this instance; required when several are
                used at once, one per wavelength (add by cjy).
        """
...
class HDOEBranchConstraintResiduals(HDOEPhaseResiduals):
    """Constrain adjacent zone-mean system OPLs to one harmonic period.

    This uses the complete traced OPL to a common image point rather than a
    thin-interface height approximation.  With fixed zone boundaries, the
    quadrature-defined constraints are differentiable continuous functions of
    the lens variables and can therefore enter the existing constrained LM step.
    """
...
class CoherentWavefrontResiduals(Residuals):
    """Maximize piston-invariant coherent concentration at a common image point.

    The real least-squares vector is the weighted, centered unit phasor.  Its
    squared norm is exactly ``1 - |mean(exp(1j * phase))|**2`` for every field,
    wavelength, and lens.  Unlike a wrapped phase difference, this residual is
    continuous across integer-wave branches and does not require phase unwrapping.
    """
    ...
    def compute_loss(
        ...
        # Subtracting any common piston leaves the centered phasors unchanged,
        # but keeps the trigonometric argument small enough for stable float64 AD.
